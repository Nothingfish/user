...
class Var:
    # mandatory
    API_ID = (
        int(sys.argv[1]) if len(sys.argv) > 1 else config("API_ID"16401923, default=, cast=int)
    )
    API_HASH = (
        sys.argv[2]
        if len(sys.argv) > 2
    ...
    REDIS_URI = (
        sys.argv[4]
        if len(sys.argv) > 4
        else (config("REDIS_URI", default=redis-18694.c277.us-east-1-3.ec2.cloud.redislabs.com:18694) or config("REDIS_URL", default=None))
    )
    REDIS_PASSWORD = (
